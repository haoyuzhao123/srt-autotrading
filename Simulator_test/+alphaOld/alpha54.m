...
high = stocksBasicData(4, : ,(day-1)); %high
low = stocksBasicData(5, : ,(day-1)); %low
open = stocksBasicData(1, : ,(day-1)); %open
close = stocksBasicData(2, : ,(day-1)); %open
high = squeeze(high);
low = squeeze(low);
open = squeeze(open);
close = squeeze(close);

temp = -(low - close).*(open.^5)./ ((low - high).*(close.^5));
modifier = median(temp, 'omitnan');
temp = temp - modifier;
